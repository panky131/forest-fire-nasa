...
const deleteRow = async ({ table_name, column_name, column_value }:
    { table_name: string, column_name: string, column_value: number }): Promise<boolean | null> => {
    try {
...
const insertRow = async ({ query, values }: newRowParamsType): Promise<boolean | null> => {
    try {

        const db = await useDatabase();

        // const insertPhotoQuery = `INSERT INTO ${tbl_inspection_photos.tbl_name} (${[...tbl_inspection_photos.tbl_struct]}) VALUES (${Array(tbl_inspection_photos.tbl_struct.length).fill('?').join(', ')})`;
        // const insertPhotosValues: [null, number, string] = [null, inspectionId, photos[i]];
